...
def pretrain(labeled_ratio=0.1):
    ...
    command = 'CUDA_VISIBLE_DEVICES=%d python pretrain_votenet.py' %GPU_ID \
        ...
        + ' --log_dir=./log_kitti/pvrcnn_%.1f' %labeled_ratio \
        + ' --print_interval=20'  \
        + ' --eval_interval=9' \
...
def train(labeled_ratio=0.1):
    ...
    command = 'CUDA_VISIBLE_DEVICES=%d python train_sess.py' %GPU_ID \
        + ' --dataset=kitti' \
        + ' --labeled_sample_list=kitti_v1_train_%.1f.txt'  %labeled_ratio \
        + ' --detector_checkpoint=./log_kitti/pvrcnn_%.1f/checkpoint.tar' %labeled_ratio \
        + ' --log_dir=./log_kitti/sess_%.1f' %labeled_ratio \
        + ' --print_interval=100'  \
        + ' --eval_interval=5' \
...
def eval_inductive(labeled_ratio=0.1):
    ...
    command = 'CUDA_VISIBLE_DEVICES=%d python eval.py' %GPU_ID \
        + ' --dataset=kitti' \
        + ' --labeled_sample_list=kitti_v1_train_%.1f.txt'  %labeled_ratio \
        + ' --checkpoint_path=' + './log_kitti/sess_%.1f/checkpoint.tar' %labeled_ratio \
        + ' --dump_dir=./dump_kitti/sess_%.1f' %labeled_ratio \
        + ' --use_3d_nms'  \
        + ' --use_cls_nms' \
        + ' --per_class_proposal' \
        + ' --dump_files' \
