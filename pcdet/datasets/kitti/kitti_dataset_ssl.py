...
class KittiDatasetSSL(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        if not self.training:  # read test data
    ...
    def get_fov_flag(pts_rect, img_shape, calib):
        """
        judge whether the point can be used(in the range of image)
        Args:
            pts_rect:
            img_shape:
            calib:
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                loc_lidar[:, 2] += h[:, 0] / 2
                # refer to https://github.com/jjw-DL/OpenPCDet-Noted/blob/master/pcdet/datasets/kitti/kitti_dataset.py
                ...
                if count_inside_pts:
                    ...
                    for k in range(num_objects):
                        # judge whether the points in the bounding box
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        for k in range(len(infos)):
            ...
            for i in range(num_obj):
                ...
                gt_points[:, :3] -= gt_boxes[i, :3]  # turn to local coordinates
    ...
    def prepare_data(self, data_dict, no_db_sample=False):  # default unlabeled data
        """
        Args:
            data_dict:
                points: (N, 3 + C_in)
        ...
        if self.training:
            ...
            # data augmentation
            gt_boxs, points, _ = random_flip_along_x(data_dict['gt_boxes'], data_dict['points'],
                                                     enable_=data_dict['flip_x'])
            gt_boxs, points, _ = random_flip_along_y(data_dict['gt_boxes'], data_dict['points'],
                                                     enable_=data_dict['flip_y'])
            gt_boxs, points, noise_scale = global_scaling(data_dict['gt_boxes'], data_dict['points'], [0.85, 1.15]
                                                          )
            gt_boxs, points, noise_rotation = global_rotation(data_dict['gt_boxes'], data_dict['points'], [-30, 30]
                                                              )
            # print(data_dict)
        ...
        if data_dict.get('gt_boxes', None) is not None:
            ...
            # feed selected samples to teacher network
            if self.training:
            ...
            if self.training:
                gt_boxes_ema = np.concatenate((data_dict['gt_boxes_ema'], gt_classes.reshape(-1, 1).astype(np.float32)),
                                              axis=1)
                data_dict['gt_boxes_ema'] = gt_boxes_ema
